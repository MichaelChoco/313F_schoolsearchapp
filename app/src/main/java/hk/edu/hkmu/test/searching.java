...
public class searching extends AppCompatActivity {
    ...
    static void ensearchcrit(ArrayList<String> list) {
        ...
        Object[] strarray = SchoolInfo.eninfoList.get(0).keySet().toArray();
        for (int i = 0; i < SchoolInfo.eninfoList.size(); i++) {
            if (SchoolInfo.eninfoList.get(i).get("Category").toLowerCase().equals(list.get(0).toLowerCase())
                    && SchoolInfo.eninfoList.get(i).get("Students Gender").toLowerCase().equals(list.get(1).toLowerCase())
                    && SchoolInfo.eninfoList.get(i).get("Session").toLowerCase().equals(list.get(2).toLowerCase())
                    && SchoolInfo.eninfoList.get(i).get("District").toLowerCase().equals(list.get(3).toLowerCase())
                    && SchoolInfo.eninfoList.get(i).get("Finance Type").toLowerCase().equals(list.get(4).toLowerCase())
                    && SchoolInfo.eninfoList.get(i).get("School Level").toLowerCase().equals(list.get(5).toLowerCase())
                    && SchoolInfo.eninfoList.get(i).get("Religion").toLowerCase().equals(list.get(6).toLowerCase())){  //check if text match any school name
                HashMap<String, String> search = new HashMap<>();
                for (int j = 0; j < SchoolInfo.eninfoList.get(0).keySet().toArray().length; j++) {
                    search.put((String) (strarray[j]), SchoolInfo.eninfoList.get(i).get(SchoolInfo.eninfoList.get(i).keySet().toArray()[j])); //using another arraylist to store the filtered result
                }
    ...
    static void chsearchcrit(ArrayList<String> list){
        ...
        Object[] strarray = SchoolInfo.chinfoList.get(0).keySet().toArray();
        for (int i = 0; i < SchoolInfo.chinfoList.size(); i++) {
            if (SchoolInfo.chinfoList.get(i).get("類別").equals(list.get(0))
                    && SchoolInfo.chinfoList.get(i).get("就讀學生性別").equals(list.get(1))
                    && SchoolInfo.chinfoList.get(i).get("學校授課時間").equals(list.get(2))
                    && SchoolInfo.chinfoList.get(i).get("分區").equals(list.get(3))
                    && SchoolInfo.chinfoList.get(i).get("資助種類").equals(list.get(4))
                    && SchoolInfo.chinfoList.get(i).get("學校類型").equals(list.get(5))
                    && SchoolInfo.chinfoList.get(i).get("宗教").equals(list.get(6))){  //check if text match any school name
                HashMap<String, String> search = new HashMap<>();
                for (int j = 0; j < SchoolInfo.chinfoList.get(0).keySet().toArray().length; j++) {
                    search.put((String) (strarray[j]), SchoolInfo.chinfoList.get(i).get(SchoolInfo.chinfoList.get(i).keySet().toArray()[j])); //using another arraylist to store the filtered result
                }
